...
#define READ_VARIABLE_FROM_ARRAY(_var_, _size_, _count_, _array_, _cleanup_code_) \
    do { \
        ...
        if ((result = memread((_var_), size, count, (_array_))) != count) { \
            PRINTF_STDERR("Error reading %s from %s. Expected count = %u. Actual count = %u.\n", (#_var_), (#_array_), (unsigned int) count, (unsigned int) result); \
            do { \
                _cleanup_code_ \
            } while (0); \
            return FILE_IO_ERROR; \
...
int read_vardump_from_file(const char * const filename,
    ...
    MALLOC_READ_VARIABLE_FROM_FILE(double_t *, raw_data, sizeof(double_t), (*num_vectors) * (*vector_dims), filename, fp, free(raw_data););
#ifndef __AUTOESL__
    *data = *raw_data;
#else
...
int read_vardump_from_array(const unsigned char ** const variable,
                            size_t * const num_vectors,
                            size_t * const vector_dims,
                            double_in_t ** const data,
                            size_t * const k, size_t * const N,
                            ...
                            double_t ** const outlier_scores) {
    /* num_vectors */
    READ_VARIABLE_FROM_ARRAY(num_vectors, sizeof(size_t), 1, variable, /* no cleanup code */);
    
    /* vector_dims */
    READ_VARIABLE_FROM_ARRAY(vector_dims, sizeof(size_t), 1, variable, /* no cleanup code */);
    ...
    MALLOC_READ_VARIABLE_FROM_ARRAY(double_t *, raw_data, sizeof(double_t), (*num_vectors) * (*vector_dims), variable, free(raw_data););
#ifndef __AUTOESL__
    *data = *raw_data;
#else
    *data = (double_t *) malloc((*num_vectors) * (*vector_dims) * sizeof(double_in_t));
    ...
    /* k */
    READ_VARIABLE_FROM_ARRAY(k, sizeof(size_t), 1, variable, free(*data););
    
    /* N */
    READ_VARIABLE_FROM_ARRAY(N, sizeof(size_t), 1, variable, free(*data););
    
    /* block_size */
    READ_VARIABLE_FROM_ARRAY(block_size, sizeof(size_t), 1, variable, free(*data););
    
    /* outliers */
    MALLOC_READ_VARIABLE_FROM_ARRAY(index_t *, outliers, sizeof(index_t), (*N), variable, free(*data););
    
    /* outlier_scores */
    MALLOC_READ_VARIABLE_FROM_ARRAY(double_t *, outlier_scores, sizeof(double_t), (*N), variable, free(*data); free(*outliers););
