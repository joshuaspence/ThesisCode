...
use constant OUTPUT_HEADER => "Profile,Data set,Iteration,Function name,Calls,Calls (relative),Self time,Self time (relative),Function time,Function time (relative),Proportion,Proportion (relative)\n";
...
for my $dataset_dir (@dataset_dirs) {
        ...
        for my $profile_dir (@profile_dirs) {
            ...
            while (my $line = <GPROF>) {
                ...
                if ($data_active && $line =~ m/^\s*(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(.+)$/) {
                    ...
                    $results    {$profile}{'functiontime'}{$function} += $self_time;
                    $all_results{$profile}{'functiontime'}{$function} += $self_time;
                    $results    {$profile}{'total_functiontime'}      += $self_time;
                    $all_results{$profile}{'total_functiontime'}      += $self_time;
    ...
    foreach my $profile (@profiles) {
        ...
        foreach my $function (@functions) {
            ...
            my $functiontime_average  = $results{$profile}{'functiontime'}{$function} / $results{$profile}{'iterations'};
            my $functiontime_relative = $results{$profile}{'functiontime'}{$function} / ($results{$profile}{'total_functiontime'} + EPSILON);
            ...
            print "\"$profile\",\"$dataset\",\"average\",\"$function\",$calls_average,$calls_relative,$selftime_average,$selftime_relative,$functiontime_average,$functiontime_relative,$proportion_average,$proportion_relative\n";
        }
    }
}
