...
plot \
    $(for i in ${ALL_BLOCKSIZES[*]}; do \
        ...
        \
        echo "'$DATA_FILE' using $COL_VECTORS:(\$$COL_BLOCKSIZE == $BLOCKSIZE_VALUE ? \$$COL_TOTALTIME : 1/0) title '$BLOCKSIZE_NAME' with linespoints lt 1 lc $COLOUR, \\"; \
    done)
    1/0
...
plot \
    $(for i in ${ALL_BLOCKSIZES[*]}; do \
        ...
        COLOUR=${BLOCKSIZE_COLOURS[$i]}; \
        \
        echo "'$DATA_FILE' using $COL_VECTORS:(\$$COL_BLOCKSIZE == $BLOCKSIZE_VALUE ? \$$COL_FUNCTIME : 1/0) title '$BLOCKSIZE_NAME' with linespoints lt 1 lc $COLOUR, \\"; \
    done)
    1/0
EOF
